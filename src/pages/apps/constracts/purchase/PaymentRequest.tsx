  ...
  const columns = useMemo<ColumnDef<Dynamic>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          return new Intl.NumberFormat('vi-VN').format(row.original.quantity);
        }
      },
      {
        header: 'Đơn giá',
      ...
      {
        header: 'Thời gian tạm ứng',
        accessorKey: 'advancePaymentDate'
      },
      {
        header: 'Tên nhà cung cấp dịch vụ',
      ...
      {
        header: 'Đơn giá dịch vụ',
        accessorKey: 'servicePrice',
        cell: ({ row }) => {
          return new Intl.NumberFormat('vi-VN', {
            style: 'currency',
            currency: row.original.currency
          }).format(row.original.servicePrice);
        }
      },
      {
        header: 'Tổng số tiền',
