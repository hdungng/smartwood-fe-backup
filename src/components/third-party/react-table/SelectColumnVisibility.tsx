    ...
    <FormControl sx={{ width: 200 }}>
      <Select
        ...
        renderValue={() => {
          if (getIsAllColumnsVisible()) {
            return <Typography variant="subtitle1">{intl.formatMessage({ id: 'all_columns_visible_label' })}</Typography>;
          }

          if (getVisibleLeafColumns().length === 0) {
            return <Typography variant="subtitle1">{intl.formatMessage({ id: 'all_columns_hidden_label' })}</Typography>;
          }

          return (
            <Typography variant="subtitle1">
              {intl.formatMessage({ id: 'column_visible_label' }, { value: getVisibleLeafColumns().length })}
            </Typography>
          );
        }}
        MenuProps={MenuProps}
      ...
      >
        <MenuItem value="all" onClick={getToggleAllColumnsVisibilityHandler()}>
          ...
          />
          <ListItemText primary={intl.formatMessage({ id: 'all_column' })} />
        </MenuItem>
        {getAllColumns().map(
          (column) =>
