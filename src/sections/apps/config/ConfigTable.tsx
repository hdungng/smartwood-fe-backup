...
// ==============================|| FILTER FIELDS ||============================== //
const filterFields = [
  { key: 'code', label: 'Mã cấu hình', type: 'text' as const },
  { key: 'name', label: 'Tên cấu hình', type: 'text' as const },
  { key: 'codeType', label: 'Loại mã', type: 'text' as const },
  { key: 'screenName', label: 'Tên màn hình', type: 'text' as const },
  {
    key: 'status',
    label: 'Trạng thái',
...
function ConfigTable({ data, columns, filters, loading, onRefresh }: Props) {
  ...
  const filteredData = useMemo(() => {
    if (activeTab === groupLabel.ALL) {
      ...
      return data.filter((config) => config.status === 1);
    } else {
      return data.filter((config) => config.status === 0);
    }
  }, [data, activeTab]);
  ...
  const handleRowDoubleClick = useCallback(
    (config: Config) => {
      ...
      // TODO: Replace with actual permission strings
      ...
      if (!hasEditAccess && !hasViewAccess) {
        ...
        openSnackbar({
          open: true,
          message: 'Bạn không có quyền xem thông tin cấu hình này',
          variant: 'alert',
          alert: {
            color: 'error',
            variant: 'filled'
      ...
      if (hasEditAccess) {
        // Full edit access
        navigate(`/system/config/edit/${config.id}`);
      } else if (hasViewAccess) {
        // View-only access
        navigate(`/system/config/view/${config.id}`);
      }
    },
    [navigate]
  );
  ...
  const csvHeaders = useMemo(
    () => [
      { label: 'ID', key: 'id' },
      { label: 'Mã cấu hình', key: 'code' },
      { label: 'Tên cấu hình', key: 'name' },
      { label: 'Mô tả', key: 'description' },
      { label: 'Loại mã', key: 'codeType' },
      { label: 'Trạng thái', key: 'status' },
      { label: 'Screen Names', key: 'screenNames' },
      { label: 'Số lượng dữ liệu', key: 'dataCount' },
  ...
  const exportData = useMemo(() => {
    const dataToExport = selectedData.length === 0 ? filteredData : selectedData;

    // Clean data to prevent CSV export errors
    return dataToExport
      .filter((config) => config && typeof config === 'object') // Filter out invalid data
      .map((config) => ({
        id: String(config.id || ''),
        code: String(config.code || ''),
        name: String(config.name || ''),
        description: String(config.description || ''),
        codeType: String(config.codeType || ''),
        status: config.status === 1 ? 'Hoạt động' : 'Không hoạt động',
        screenNames: Array.isArray(config.metaData?.screenNames) ? config.metaData.screenNames.join(', ') : '',
        dataCount: Array.isArray(config.data) ? config.data.length : 0,
        createdAt: String(config.createdAt || ''),
        updatedAt: String(config.updatedAt || '')
      }));
  }, [selectedData, filteredData]);

  const searchPlaceholder = useMemo(() => `Tìm kiếm ${filteredData.length} cấu hình...`, [filteredData.length]);
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={responsiveStyles.container}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} sx={responsiveStyles.actionStack}>
          ...
          <Button
            ...
            onClick={() => navigate('/system/config/create')}
            style={{
              padding: '9px 12px'
            }}
          >
          ...
          <CSVExport data={exportData} filename={`configs-${new Date().getTime()}.csv`} headers={csvHeaders} />
        </Stack>
      </Stack>
      ...
      <ScrollX>
        <Stack>
          ...
          >
            <Table>
              <TableHead>
                {table.getHeaderGroups().map((headerGroup: HeaderGroup<Config>) => (
                ...
                {filteredData.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={table.getAllColumns().length}>
                      <EmptyTable msg="Không có dữ liệu cấu hình" />
                    </TableCell>
                  </TableRow>
                ) : (
