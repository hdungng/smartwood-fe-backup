...
interface Props {
  data: TShippingUnit[];
  columns: ColumnDef<TShippingUnit>[];
  loading?: boolean;
  filters?: {
    search: string;
...
// ==============================|| SHIPPING UNIT TABLE COMPONENT ||============================== //
function ShippingUnitTable({ data, columns, filters }: Props) {
  ...
  const counts = useMemo(() => {
    return data.reduce(
      (acc, shippingUnit) => {
        if (shippingUnit.status === 1) {
          acc.Active += 1;
        } else if (shippingUnit.status === 0) {
          acc.Inactive += 1;
        }
        return acc;
      },
  ...
  const filteredData = useMemo(() => {
    if (activeTab === groupLabel.ALL) {
      ...
      return data.filter((shippingUnit) => shippingUnit.status === 1);
    } else {
      return data.filter((shippingUnit) => shippingUnit.status === 0);
    }
  }, [data, activeTab]);
  ...
  const handleRowDoubleClick = useCallback(
    (shippingUnit: TShippingUnit) => {
      // Check permissions for shipping unit management
      const hasEditAccess = hasPermission(['M_SHIPPING_UNIT_UPDATE'])
      const hasViewAccess = hasPermission(['M_SHIPPING_UNIT_VIEW'])

      if (!hasEditAccess && !hasViewAccess) {
        // No permission - show error message
        openSnackbar({
          open: true,
          message: 'Bạn không có quyền xem thông tin đơn vị vận chuyển này',
          variant: 'alert',
          alert: {
            color: 'error',
            variant: 'filled'
      ...
      if (hasEditAccess) {
        // Full edit access
        navigate(`/master/shipping-unit/edit/${shippingUnit.id}`);
      } else if (hasViewAccess) {
        // View-only access
        navigate(`/master/shipping-unit/view/${shippingUnit.id}`);
      }
    },
    [navigate]
  );
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={responsiveStyles.container}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} sx={responsiveStyles.actionStack}>
          ...
          <Button
            ...
            onClick={() => navigate('/master/shipping-unit/create')}
            style={{
              padding: '9px 12px'
            }}
          >
          ...
          <CSVExport data={exportData} headers={csvHeaders} filename="shipping-unit-list.csv" />
        </Stack>
      </Stack>
      ...
      <ScrollX>
        <Stack>
          ...
          >
            <Table>
              <TableHead>
                {table.getHeaderGroups().map((headerGroup: HeaderGroup<TShippingUnit>) => (
...
export default memo(ShippingUnitTable);
