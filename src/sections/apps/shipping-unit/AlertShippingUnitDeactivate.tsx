...
export default function AlertShippingUnitDeactivate({ id, title, open, handleClose, onDeactivateSuccess }: Props) {
  const { deactivate: deactivateShippingUnit } = useShippingUnit();

  const handleDeactivate = async () => {
    try {
      await deactivateShippingUnit(id);
        ...
        <Stack direction="row" alignItems="center" spacing={1.5}>
          ...
          <Box>
            <Typography variant="h5" component="div" sx={{ fontWeight: 700 }}>
              Ngừng hoạt động đơn vị vận chuyển
            </Typography>
      ...
      <DialogContent sx={{ pt: 2, pb: 3 }}>
        <DialogContentText sx={{ mb: 2 }}>
          <Typography variant="h6" sx={{ mb: 2, fontWeight: 600, color: 'text.primary' }}>
            Bạn có chắc chắn muốn ngừng hoạt động đơn vị vận chuyển{' '}
          ...
          >
            <Typography variant="body1" color="text.primary" sx={{ fontWeight: 600 }}>
              ...
              <br />
              Hành động này sẽ chuyển đơn vị vận chuyển sang trạng thái không hoạt động và nó sẽ xuất hiện trong tab{' '}
