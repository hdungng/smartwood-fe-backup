...
interface Props {
  data: TPaymentTerm[];
  columns: ColumnDef<TPaymentTerm>[];
  loading?: boolean;
  filters?: {
    search: string;
...
// ==============================|| PAYMENT TERM TABLE COMPONENT ||============================== //
function PaymentTermTable({ data, columns, filters }: Props) {
  ...
  const counts = useMemo(() => {
    return data.reduce(
      (acc, paymentTerm) => {
        if (paymentTerm.status === 1) {
          acc.Active += 1;
        } else if (paymentTerm.status === 0) {
          acc.Inactive += 1;
        }
        return acc;
      },
  ...
  const filteredData = useMemo(() => {
    if (activeTab === groupLabel.ALL) {
      ...
      return data.filter((paymentTerm) => paymentTerm.status === 1);
    } else {
      return data.filter((paymentTerm) => paymentTerm.status === 0);
    }
  }, [data, activeTab]);
  ...
  const handleRowDoubleClick = useCallback(
    (paymentTerm: TPaymentTerm) => {
      const hasEditAccess = hasPermission(['M_PAYMENT_TERM_UPDATE'])
      const hasViewAccess = hasPermission(['M_PAYMENT_TERM_VIEW']);

      if (!hasEditAccess && !hasViewAccess) {
        openSnackbar({
          open: true,
          message: 'Bạn không có quyền xem thông tin điều kiện thanh toán này',
          variant: 'alert',
          alert: {
            color: 'error',
            variant: 'filled'
      ...
      if (hasEditAccess) {
        navigate(`/master/payment-term/edit/${paymentTerm.id}`);
      } else if (hasViewAccess) {
        navigate(`/master/payment-term/view/${paymentTerm.id}`);
      }
    },
    [navigate]
  );
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={responsiveStyles.container}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} sx={responsiveStyles.actionStack}>
          ...
          <Button
            ...
            onClick={() => navigate('/master/payment-term/create')}
            style={{
              padding: '9px 12px'
            }}
          >
            Thêm điều kiện thanh toán
          </Button>
          <CSVExport data={exportData} headers={csvHeaders} filename="payment-term-list.csv" />
        </Stack>
      </Stack>
      ...
      <ScrollX>
        <Stack>
          ...
          >
            <Table>
              <TableHead>
                {table.getHeaderGroups().map((headerGroup: HeaderGroup<TPaymentTerm>) => (
...
export default memo(PaymentTermTable);
