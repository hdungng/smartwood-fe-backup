...
export default function AlertDeliveryTermActivate({ id, title, open, handleClose, onActivateSuccess }: Props) {
  const { activate } = useDeliveryTerm();
  ...
  const handleActivate = async () => {
    try {
      ...
      openSnackbar({
        open: true,
        message: `Phương thức vận chuyển"${title}" đã được kích hoạt thành công!`,
        variant: 'alert',
        alert: {
          color: 'success',
          variant: 'filled'
      ...
      openSnackbar({
        open: true,
        message: error instanceof Error ? error.message : 'Có lỗi xảy ra khi kích hoạt Phương thức vận chuyển',
        variant: 'alert',
        alert: {
          color: 'error',
          variant: 'filled'
      ...
      <DialogContent>
        <DialogContentText sx={{ mb: 2 }}>
          <Typography variant="h6" sx={{ mb: 2, fontWeight: 600, color: 'text.primary' }}>
            Bạn có chắc chắn muốn kích hoạt Phương thức vận chuyển{' '}
          ...
          >
            <Typography variant="body1" color="text.primary" sx={{ fontWeight: 600 }}>
              ...
              <br />
              Hành động này sẽ chuyển Phương thức vận chuyểnsang trạng thái hoạt động và nó sẽ xuất hiện trong tab{' '}
