...
interface Props {
  data: TDeliveryTerm[];
  columns: ColumnDef<TDeliveryTerm>[];
  loading?: boolean;
  filters?: {
    search: string;
...
// ==============================|| DELIVERY TERM TABLE COMPONENT ||============================== //
function DeliveryTermTable({ data, columns, filters }: Props) {
  ...
  const counts = useMemo(() => {
    return data.reduce(
      (acc, deliveryTerm) => {
        if (deliveryTerm.status === 1) {
          acc.Active += 1;
        } else if (deliveryTerm.status === 0) {
          acc.Inactive += 1;
        }
        return acc;
      },
  ...
  const filteredData = useMemo(() => {
    if (activeTab === groupLabel.ALL) {
      ...
      return data.filter((deliveryTerm) => deliveryTerm.status === 1);
    } else {
      return data.filter((deliveryTerm) => deliveryTerm.status === 0);
    }
  }, [data, activeTab]);
  ...
  const handleRowDoubleClick = useCallback(
    (deliveryTerm: TDeliveryTerm) => {
      // Check permissions for delivery term management
      const hasEditAccess = hasPermission('M_DELIVERY_TERM_UPDATE')
      const hasViewAccess = hasPermission('M_DELIVERY_TERM_VIEW')

      if (!hasEditAccess && !hasViewAccess) {
        // No permission - show error message
        openSnackbar({
          open: true,
          message: 'Bạn không có quyền xem thông tin Phương thức vận chuyểnnày',
          variant: 'alert',
          alert: {
            color: 'error',
            variant: 'filled'
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={responsiveStyles.container}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} sx={responsiveStyles.actionStack}>
          ...
          <Button
            ...
            onClick={() => navigate('/master/delivery-term/create')}
            style={{
              padding: '9px 12px'
            }}
          >
          ...
          <CSVExport data={exportData} headers={csvHeaders} filename="delivery-term-list.csv" />
        </Stack>
      </Stack>
      ...
      <ScrollX>
        <Stack>
          ...
          >
            <Table>
              <TableHead>
                {table.getHeaderGroups().map((headerGroup: HeaderGroup<TDeliveryTerm>) => (
...
export default memo(DeliveryTermTable);
