...
export default function AlertGoodActivate({ id, title, open, handleClose, onActivateSuccess }: Props) {
  const { activate } = useGood();
  ...
  const handleActivate = async () => {
    try {
      ...
      openSnackbar({
        open: true,
        message: `Sản phẩm "${title}" đã được kích hoạt thành công!`,
        variant: 'alert',
        alert: {
          color: 'success',
          variant: 'filled'
      ...
      openSnackbar({
        open: true,
        message: error instanceof Error ? error.message : 'Có lỗi xảy ra khi kích hoạt sản phẩm',
        variant: 'alert',
        alert: {
          color: 'error',
          variant: 'filled'
        ...
        <DialogContentText sx={{ mb: 2 }}>
          ...
          >
            <Typography variant="body1" color="text.primary" sx={{ fontWeight: 600 }}>
              ...
              <br />
              Hành động này sẽ chuyển sản phẩm sang trạng thái hoạt động và nó sẽ xuất hiện trong tab <strong>Đang hoạt động</strong>.
            </Typography>
          </Box>
        </DialogContentText>
      </DialogContent>
