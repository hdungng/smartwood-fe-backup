...
function GoodTable({ data, columns, filters }: Props) {
  ...
  const filteredData = useMemo(() => {
    if (activeTab === groupLabel.ALL) {
      ...
      return data.filter((good) => good.status === 1);
    } else {
      return data.filter((good) => good.status === 0);
    }
  }, [data, activeTab]);
  ...
  const handleRowDoubleClick = useCallback(
    (good: TGood) => {
      // Check permissions for good management
      const hasEditAccess = hasPermission(['M_GOOD_UPDATE']);
      const hasViewAccess = hasPermission(['M_GOOD_VIEW']);

      if (!hasEditAccess && !hasViewAccess) {
        // No permission - show error message
        openSnackbar({
          open: true,
          message: 'Bạn không có quyền xem thông tin good này',
          variant: 'alert',
          alert: {
            color: 'error',
            variant: 'filled'
      ...
      if (hasEditAccess) {
        // Full edit access
        navigate(`/master/good/edit/${good.id}`);
      } else if (hasViewAccess) {
        // View-only access
        navigate(`/master/good/view/${good.id}`);
      }
    },
    [navigate]
  );
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={responsiveStyles.container}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} sx={responsiveStyles.actionStack}>
          ...
          <Button
            ...
            onClick={() => navigate('/master/good/create')}
            style={{
              padding: '9px 12px'
            }}
          >
          ...
          <CSVExport data={exportData} headers={csvHeaders} filename="good-list.csv" />
        </Stack>
      </Stack>
      ...
      <ScrollX>
        <Stack>
          ...
          >
            <Table>
              <TableHead>
                {table.getHeaderGroups().map((headerGroup: HeaderGroup<TGood>) => (
