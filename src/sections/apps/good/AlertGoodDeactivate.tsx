...
export default function AlertGoodDeactivate({ id, title, open, handleClose, onDeactivateSuccess }: Props) {
  const { deactivate: deactivateGood } = useGood();

  const handleDeactivate = async () => {
    try {
      await deactivateGood(id);

      openSnackbar({
        open: true,
        message: 'Sản phẩm đã được chuyển sang trạng thái không hoạt động',
        variant: 'alert',
        alert: { color: 'success' }
      } as SnackbarProps);
      ...
      openSnackbar({
        open: true,
        message: error instanceof Error ? error.message : 'Có lỗi xảy ra khi ngừng hoạt động sản phẩm',
        variant: 'alert',
        alert: { color: 'error' }
      } as SnackbarProps);
    }
        ...
        <DialogContentText sx={{ mb: 2 }}>
          ...
          >
            <Typography variant="body1" color="text.primary" sx={{ fontWeight: 600 }}>
              ...
              <br />
              Hành động này sẽ chuyển sản phẩm sang trạng thái không hoạt động và nó sẽ xuất hiện trong tab{' '}
