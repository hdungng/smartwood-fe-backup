...
import ExpandingOrderDetail from 'sections/apps/order/ExpandingOrderDetail';

// types
import { mockColumns, mockData, OrderList } from 'types/order';
...
const fuzzyFilter: FilterFn<OrderList> = (row, columnId, value, addMeta) => {
  // rank the item
...
interface Props {
  data: OrderList[];
  columns: ColumnDef<OrderList>[];
  modalToggler: () => void;
}

// ==============================|| REACT TABLE - LIST ||============================== //

export default function OrderTable({ data = mockData, columns = mockColumns, modalToggler }: Props) {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((order) => order.suppliers.some((supplier) => supplier.mass === statusFilter));
  }, [statusFilter, data]);
            ...
            <CSVExport
              {...{
                data:
                  ...
                  table.getSelectedRowModel() && table.getSelectedRowModel().flatRows && table.getSelectedRowModel().flatRows.length > 0
                    ? table.getSelectedRowModel().flatRows.map((row) => row.original)
                    : data,
                headers,
                    ...
                    {headerGroup.headers.map((header) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* key={header.id}
                          {...(header.column.columnDef.meta || {})} // Đảm bảo meta không undefined
                          onClick={header.column.getCanSort() ? header.column.getToggleSortingHandler() : undefined} // Chỉ thêm handler nếu có thể sắp xếp
                          className={
                            header.column.getCanSort() && !header.column.columnDef.meta ? 'cursor-pointer prevent-select' : undefined
                          }
                        >
                    ...
                    {row.getIsExpanded() && (
                      ...
                      >
                        <TableCell colSpan={row.getVisibleCells().length}>
                          <ExpandingOrderDetail data={row.original} />
                        </TableCell>
                      </TableRow>
                    )}
                  </Fragment>
