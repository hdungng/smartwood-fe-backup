...
export default function AlertSupplierActivate({ id, title, open, handleClose, onActivateSuccess }: Props) {
  const { activate } = useSupplier();
  ...
  const handleActivate = async () => {
    try {
      ...
      openSnackbar({
        open: true,
        message: `Nhà cung cấp "${title}" đã được kích hoạt thành công!`,
        variant: 'alert',
        alert: {
          color: 'success',
          variant: 'filled'
      ...
      openSnackbar({
        open: true,
        message: error instanceof Error ? error.message : 'Có lỗi xảy ra khi kích hoạt nhà cung cấp',
        variant: 'alert',
        alert: {
          color: 'error',
          variant: 'filled'
      ...
      <DialogContent>
        <DialogContentText component="div" sx={{ mb: 2 }}>
          <Typography variant="h6" sx={{ mb: 2, fontWeight: 600, color: 'text.primary' }}>
            Bạn có chắc chắn muốn kích hoạt nhà cung cấp{' '}
          ...
          >
            <Typography variant="body1" color="text.primary" sx={{ fontWeight: 600 }}>
              ...
              <br />
              Hành động này sẽ chuyển nhà cung cấp sang trạng thái hoạt động và nó sẽ xuất hiện trong tab <strong>Đang hoạt động</strong>.
            </Typography>
          </Box>
        </DialogContentText>
      </DialogContent>
