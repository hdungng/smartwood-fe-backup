...
export default function AlertSupplierDeactivate({ id, title, open, handleClose, onDeactivateSuccess }: Props) {
  const { deactivate: deactivateSupplier } = useSupplier();
  ...
  const handleDeactivate = async () => {
    try {
      ...
      openSnackbar({
        open: true,
        message: 'Supplier has been moved to inactive status',
        variant: 'alert',
        alert: { color: 'success' }
      } as SnackbarProps);
      ...
      openSnackbar({
        open: true,
        message: error instanceof Error ? error.message : 'Failed to deactivate supplier',
        variant: 'alert',
        alert: { color: 'error' }
      } as SnackbarProps);
    }
      ...
      <DialogContent sx={{ pt: 2, pb: 3 }}>
        <DialogContentText component="div" sx={{ mb: 2 }}>
          <Typography variant="h6" sx={{ mb: 2, fontWeight: 600, color: 'text.primary' }}>
            Bạn có chắc chắn muốn ngừng hoạt động nhà cung cấp{' '}
          ...
          >
            <Typography variant="body1" color="text.primary" sx={{ fontWeight: 600 }}>
              ...
              <br />
              Hành động này sẽ chuyển nhà cung cấp sang trạng thái không hoạt động và nó sẽ xuất hiện trong tab{' '}
