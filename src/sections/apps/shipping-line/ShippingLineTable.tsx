...
interface Props {
  data: ShippingLine[];
  columns: ColumnDef<ShippingLine>[];
  loading?: boolean;
  filters?: {
    search: string;
...
// ==============================|| SHIPPING LINE TABLE COMPONENT ||============================== //
function ShippingLineTable({ data, columns, filters }: Props) {
  ...
  const counts = useMemo(() => {
    return data.reduce(
      (acc, shippingLine) => {
        if (shippingLine.status === 1) {
          acc.Active += 1;
        } else if (shippingLine.status === 0) {
          acc.Inactive += 1;
        }
        return acc;
      },
  ...
  const filteredData = useMemo(() => {
    if (activeTab === groupLabel.ALL) {
      ...
      return data.filter((shippingLine) => shippingLine.status === 1);
    } else {
      return data.filter((shippingLine) => shippingLine.status === 0);
    }
  }, [data, activeTab]);
  ...
  const handleRowDoubleClick = useCallback(
    (shippingLine: ShippingLine) => {
      // Check permissions for shipping line management
      const hasEditAccess = hasPermission(['M_SHIPPING_LINE_UPDATE'])
      const hasViewAccess = hasPermission(['M_SHIPPING_LINE_VIEW'])

      if (!hasEditAccess && !hasViewAccess) {
        // No permission - show error message
        openSnackbar({
          open: true,
          message: 'Bạn không có quyền xem thông tin shipping line này',
          variant: 'alert',
          alert: {
            color: 'error',
            variant: 'filled'
      ...
      if (hasEditAccess) {
        // Full edit access
        navigate(`/master/shipping-line/edit/${shippingLine.id}`);
      } else if (hasViewAccess) {
        // View-only access
        navigate(`/master/shipping-line/view/${shippingLine.id}`);
      }
    },
    [navigate]
  );
  ...
  const searchPlaceholder = useMemo(() => `Tìm kiếm ${filteredData.length} shipping line...`, [filteredData.length]);
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={responsiveStyles.container}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} sx={responsiveStyles.actionStack}>
          ...
          <Button
            ...
            onClick={() => navigate('/master/shipping-line/create')}
            style={{
              padding: '9px 12px'
            }}
          >
          ...
          headers={csvHeaders} 
          filename="danh-sach-shipping-line.csv"
           />
        </Stack>
      </Stack>
      ...
      <ScrollX>
        <Stack>
          ...
          >
            <Table>
              <TableHead>
                {table.getHeaderGroups().map((headerGroup: HeaderGroup<ShippingLine>) => (
...
export default memo(ShippingLineTable);
